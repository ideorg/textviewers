...
TEST (ProportionalL, portion) {
    ...
    for (int j = 0; j <= doc.lineCount(); j ++)
        for (int i = 20; i < 40; i += 5) {
            vtest.setScreenLineCount(i);
            vtest.gotoProportional((double)j/doc.lineCount());
            vtest.fillDeque();
            vtest.recalcLines();
...
TEST (ProportionalL, reversing) {
    ...
    for (int i = 0; i < doc.lineCount(); i += 10) {
        vtest.gotoProportional((double)i/doc.lineCount());
        vtest.fillDeque();
        vtest.recalcLines();
